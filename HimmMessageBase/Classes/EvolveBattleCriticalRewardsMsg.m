//
//  EvolveBattleCriticalRewardsMsg.m
//  Runner
//
//  Created by FY0125 on 2023/10/21.
//

#import "EvolveBattleCriticalRewardsMsg.h"

@implementation EvolveBattleCriticalRewardsMsg
+ (instancetype)messageWithContent:(NSString *)jsonContent {
    EvolveBattleCriticalRewardsMsg *data = [EvolveBattleCriticalRewardsMsg new];
    if (data) {
...
+(NSString *)getObjectName {
    return @"HM:BattleCriticalRewardsMsg";
}

//最后一条消息是自定义消息的时候，可以更改在会话列表显示的类型，为了区分消息类型
- (NSString *)conversationDigest {
    NSString *contentStr = @"[pk]";
    return contentStr;
}
@end
