//
//  EvolveDuelOfficialSystemMessage.m
//  Runner
//
//  Created by himi on 11/08/23.
//

#import "EvolveDuelOfficialSystemMessage.h"

@implementation EvolveDuelOfficialSystemMessage

+ (instancetype)messageWithContent:(NSString *)jsonContent {
    EvolveDuelOfficialSystemMessage *data = [EvolveDuelOfficialSystemMessage new];
    if (data) {
...
+(NSString *)getObjectName {
    return @"HM:OfficialSystemMsg";
}

//最后一条消息是自定义消息的时候，可以更改在会话列表显示的类型，为了区分消息类型
- (NSString *)conversationDigest {
    NSString *contentStr = @"[官方连麦消息]";
    return contentStr;
}
