//
//  EvolveDuelUserChangeMessage.m
//  Runner
//
//  Created by himi on 11/08/23.
//

#import "EvolveDuelUserChangeMessage.h"

@implementation EvolveDuelUserChangeMessage

+ (instancetype)messageWithContent:(NSString *)jsonContent {
    EvolveDuelUserChangeMessage *facekData = [EvolveDuelUserChangeMessage new];
    if (facekData) {
...
+(NSString *)getObjectName {
    return @"HM:PKInfoUserChange";
}

//最后一条消息是自定义消息的时候，可以更改在会话列表显示的类型，为了区分消息类型 pk排行推送
- (NSString *)conversationDigest {
    NSString *contentStr = @"[pk refreshuser]";
    return contentStr;
}
