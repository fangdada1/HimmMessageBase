//
//  EvolveFansBeginLiveMessage.m
//  Runner
//
//  Created by himi on 11/08/23.
//

#import "EvolveFansBeginLiveMessage.h"

@implementation EvolveFansBeginLiveMessage

+ (instancetype)messageWithContent:(NSString *)jsonContent {
    EvolveFansBeginLiveMessage *data = [EvolveFansBeginLiveMessage new];
    if (data) {
...
+(NSString *)getObjectName {
    return @"HM:FansBeginLiveMsg";
}

//最后一条消息是自定义消息的时候，可以更改在会话列表显示的类型，为了区分消息类型
- (NSString *)conversationDigest {
    NSString *contentStr = @"[关注的人开播了]";
    return contentStr;
}
